...
This scraper extracts job listings from Codespring careers page (Romania only) and imports them to peviitor.ro.

Target: https://www.codespring.ro/category/jobs/feed (WordPress RSS Feed)
...
1. **Start with brand** - We know the brand (e.g., "SOFTECH SRL")
...
8. **Scrape new jobs** - Extract jobs from Codespring WordPress RSS feed
...
set SOLR_AUTH=your-solr-credentials
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Codespring jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Codespring RSS feed
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Codespring RSS feed (jobs)
    │
    ▼
transformJobsForSOLR()
    ├── Filter: keep only Romanian locations
    │         (Cluj-Napoca, Târgu Mureș, etc)
...
| File | Role |
|------|------|
| `index.js` | Main entry point - full workflow: extract existing → validate company → scrape → transform → upsert |
...
- **DemoANAF Search**: `https://demoanaf.ro/api/search?q=SOFTECH%20SRL` - Search companies by name/brand
